...
class PPOTrainer:
    def __init__(self, config:dict, run_id:str="run", device:torch.device=torch.device("cpu")) -> None:
        ...
        self.reward_lst = np.zeros(self.config["n_workers"])
        ...
        #observation_space = dummy_env.observation_space[0] #one_agent
        observation_space = np.zeros((3,13,13)) #one_agent
        action_space_shape = (dummy_env.action_space[0].n,) #one_agent
        dummy_env.close()
    ...
    def run_training(self) -> None:
        ...
        for update in range(self.config["updates"]):
            ...
            result = "{:4} reward={:.2f} max={:.2f} min={:.1f}  pi_loss={:3f} v_loss={:3f} entropy={:.3f} loss={:3f} value={:.3f} advantage={:.3f}".format(
                update, self.reward_lst.mean(), max(self.reward_lst), min(self.reward_lst), 
    ...
    def _sample_training_data(self) -> list:
        ...
        for t in range(self.config["worker_steps"]):
            ...
            for w, worker in enumerate(self.workers):
                worker.child.send(("step", [self.buffer.actions[w, t].cpu().numpy()])) #one_agent listing
            # Retrieve step results from the environments
            for w, worker in enumerate(self.workers):
                obs, reward, done, info = worker.child.recv()
                reward = reward[0] #one_agent
                obs = obs[0] #one_agent
                self.buffer.rewards[w, t], self.buffer.dones[w, t] = reward * self.config["reward_scale"], done
                #if info:
                if done:
                    ...
                    worker.child.send(("reset", None))
                    # Get data from reset
                    obs,total_reward = worker.child.recv()
                    obs = obs[0] #one_agent
                    self.reward_lst[w] = total_reward
    ...
    def _write_training_summary(self, update, training_stats) -> None: #, episode_result
        ...
        self.writer.add_scalar("training/reward_mean", np.mean(self.reward_lst), update)
        self.writer.add_scalar("training/max_mean", np.max(self.reward_lst), update)
        self.writer.add_scalar("training/min_mean", np.min(self.reward_lst), update)
